 ...
 *
 * Detection order:
 * 1. UTF-16 BOM (LE/BE) → certain
 * 2. UTF-8 BOM → certain
 * 3. Valid UTF-8 with multi-byte sequences → high confidence
 * 4. None of the above → unknown (caller provides encoding)
 */
...
export type SniffedEncoding = "utf8" | "utf16le" | "utf16be" | "unknown";
 ...
 * 1. UTF-16 BOM (LE/BE) → certain
 * 2. UTF-8 BOM → certain
 * 3. Valid UTF-8 with multi-byte content → assumed UTF-8 (no BOM).
 *    A buffer that decodes as valid UTF-8 *and* contains at least one
 *    non-ASCII character is almost certainly UTF-8 — it is vanishingly
 *    unlikely that a single-byte ANSI file would happen to form valid
 *    UTF-8 multi-byte sequences.
 * 4. None of the above → `"unknown"`.  The caller should provide the
 *    encoding explicitly (e.g. via {@link getSystemCodepage}).
 *
 * See {@link ../docs/design/encoding-sniffer.md} for the full design doc.
 */
export function sniffEncoding(buffer: Buffer): SniffResult {
	...
	// ── UTF-8 without BOM (only if it has multi-byte content) ──

	const utf8Check = checkUtf8(buffer);
	if (utf8Check === "multi-byte") {
		return { encoding: "utf8", hasBom: false };
	}

	// ── Pure ASCII or invalid UTF-8 — encoding is unknown ──────
...
/**
 * Check if a buffer is valid UTF-8 and whether it contains non-ASCII content.
 *
 * @returns `"multi-byte"` — valid UTF-8 with at least one multi-byte sequence
 *          `"ascii-only"` — valid UTF-8, all bytes < 0x80
 *          `"invalid"`     — not valid UTF-8
 */
function checkUtf8(buffer: Buffer): "multi-byte" | "ascii-only" | "invalid" {
	...
	return hasMultiByte ? "multi-byte" : "ascii-only";
}
